...
app.get("/api/users", async (_, res) => {
  const r = await exec("SELECT id, name, username, password, role, \"isAdmin\" FROM users");
  res.json(r.rows.map(u => ({ ...u, isAdmin: bool(u.isAdmin) })));
});
app.post("/api/users", async (req, res) => {
  const { id, name, username, password, role, isAdmin } = req.body;
  try {
    await exec(`INSERT INTO users (id, name, username, password, role, "isAdmin") VALUES (?,?,?,?,?,?)`,
      [id, name, username, password, role, isAdmin ? 1 : 0]);
    res.json({ ok: true });
  } catch { res.status(400).json({ error: "Usuário já existe" }); }
});
app.put("/api/users/:id", async (req, res) => {
  const { name, username, password, role, isAdmin } = req.body;
  await exec(`UPDATE users SET name=?, username=?, password=?, role=?, "isAdmin"=? WHERE id=?`,
    [name, username, password, role, isAdmin ? 1 : 0, req.params.id]);
  res.json({ ok: true });
});
...
app.get("/api/bootstrap", async (_, res) => {
  try {
    const [u, i, iCount, c, t, co, rem, pi, pc] = await Promise.all([
      exec("SELECT id, name, username, password, role, \"isAdmin\" FROM users"),
